...
class AIInsights:
    ...
    def __init__(self, config, data_dir=None):
        ...
        self.model = "claude-sonnet-4-5-20250929"  # Fast + smart
    ...
    def analyze_trades(self, trade_history, performance_stats, positions,
        ...
        Args:
            ...
            performance_stats: Overall performance metrics
            positions: Current open positions dict
            regime_data: Current market regime info
            strategy_scores: Learning system's strategy scores

        Returns:
            dict with insights, recommendations, and structured adjustments
        """
        if not self.api_key:
            return {
                "available": False,
        ...
        # Prepare the trade data summary for Claude
    ...
    def _build_prompt(self, trade_history, performance, positions,
                      regime_data, strategy_scores):
        """Build the analysis prompt for Claude."""

        # Last 50 trades (keep it concise for token efficiency)
        ...
        prompt = f"""You are a professional trading coach analyzing an algo trading bot's recent performance.
...
Be direct, specific, and actionable. Use actual numbers from the data. Don't be generic - reference specific trades and patterns you see."""
    ...
    def get_quick_insight(self, trade_history, performance_stats):
        ...
        Good for periodic updates without full analysis.
        """
        if not self.api_key:
            return None
    ...
    def get_cached_insights(self):
        """Return cached insights without making an API call."""
        if self._cached_insights:
            return self._cached_insights
        # Return the most recent from history
    ...
    def _load_insights_history(self):
        """Load insights history from disk."""
