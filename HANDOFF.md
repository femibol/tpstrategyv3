...
## Last Updated
2026-05-27 (session 9) — **Four PRs merged + local-merge fixes resolved: trail-arm gate for crypto (#172), CoinGecko dynamic universe scanner (#173), bot/ bind-mount (#174), crypto_runner strategy + new-entrants scanner signal (#175). Plus the host's pre-existing local edits (stricter top_gainers thresholds + ibkr qualified-contracts cache) merged into main via conflict resolution (`0afdd38`).** Session catalyst was the user's request to review crypto trades and catch dynamic runners. Trade-history review of 173 crypto trades (2026-05-17..05-26, +$708 net at 61% WR, carried by NEAR +$578) surfaced four findings: (1) the session-5(9) trail floor (`18ae5f2`) didn't fully fix the trail leak — 12 post-fix exits below entry by 0.04-1.29%, net -$150, because the fix prevented the trail being SET below entry but allowed it AT entry the moment price ticked one print above, creating a breakeven stop that wicked out on the next slippage tick; (2) `momentum` on crypto already disabled (session 5(5)) and verified working — the 10 legacy momentum-crypto rows in trade_history all pre-date that deploy; (3) 30-60 min "death zone" (21% WR, -$146 post-fix) is largely the same trail-wick trades — collapses into fix #1; (4) 14d trend filter from session 8 working perfectly — 0 bleeder entries (ICP/DOT/BCH/SUI/LINK/AVAX/LTC/BTC/ETH) since 2026-05-25 deploy, 11 trades net +$276 in that window. **PR #172 (`deb5969`):** added `CRYPTO_TRAIL_ARM_PCT=0.005` to `_trail_arm_allowed` (engine.py:2835) requiring +0.5% pnl before the crypto trail may arm; static `stop_loss` continues to bound downside while trail is un-armed. All 4 call sites in engine.py (`_fast_scalp_monitor` 3298, `_on_5sec_bar` 3417, `_on_tick` 3518, `_monitor_positions` 4240) now pass `symbol`. 6 new tests in `test_trail_arm_crypto.py`. **PR #173 (`c752d8e`):** new `bot/data/crypto_scanner.py` hits CoinGecko's free `/coins/markets?order=volume_desc` endpoint, filters stablecoins + wrapped/staked derivatives (without the blacklist the top-N is half USDT/USDC/WBTC/STETH/...), caches 24h on disk at `data/crypto_scanner_cache.json`. New `_get_crypto_universe()` helper in engine.py UNIONS scanner output with static `crypto.symbols` list (scanner-ranked first, static favorites appended), so hand-curated favorites (NEAR/RNDR/ATOM) are never dropped by a CoinGecko ranking change; falls back to static on scanner failure so an outage never disarms the crypto sleeve. **Off by default — flip `crypto.dynamic_universe.enabled: true` in `config/settings.yaml` to activate.** 14 new tests in `test_crypto_scanner.py`. **PR #174 (`8e5feca`):** carried-over follow-up from sessions 5(8)/5(9). Added `./bot:/app/bot` bind-mount + `PYTHONDONTWRITEBYTECODE=1` to docker-compose.yml. Code changes now only need `docker restart trading-bot-trading-bot-1` instead of a full rebuild. **First restart after this PR needs `docker compose up -d --force-recreate trading-bot`** (plain `docker restart` won't re-apply volume directives). Config bind-mount turned out to already be in place (docker-compose.yml:187) — only bot/ was missing. **Local merge on host (`0afdd38`):** the user's host had pre-existing uncommitted edits blocking `git pull`. Conflict resolution kept LOCAL stricter values: top_gainers premarket vol 25K→100K, regular 100K→300K, postmarket 75K→150K, all change% 5→10%; plus ibkr.py's `_invalid_symbols_ttl` 2h→24h + new `_qualified_contracts` dict cache (init at :133, clear :1019, lookup :1026, write :1044). Reasoning for taking local: their comments referenced upstream's numbers ("raised from 25K") proving local was the LATER edit; ibkr cache was a real perf fix for the SLOW CYCLE 200s+ regression observed 2026-05-26 (re-qualifying 42 dead contracts per cycle was burning ~80s/cycle). **PR #175 (`6e896f8`):** closes the runner-catch gap exposed by tonight's review. The bot grinds dip-buys on slow uptrends via mean_reversion but had no entry path for live pumps (WLD/DRIFT-style intraday +20%). New `bot/strategies/crypto_runner.py` mirrors low_float_catalyst's "hard stop, hard TP, time stop, no trail" shape on crypto — entry 1h move ≥+5% + RVOL ≥3x (5-min bars), exit -6%/+15%/240min, no trailing (PR #172 rationale — trails on crypto pumps' pullbacks = guaranteed nicks). New `crypto_scanner.new_entrants()` returns symbols that JUST appeared in CoinGecko top-50 vs prior snapshot; strategy halves the 1h-change floor for new entrants (default 0.5x → 2.5%) so the first 30 min of a run isn't missed. Allocation 4% in strategies.yaml (~$960 @ $24K account), added to `crypto.allowed_strategies` so engine injection feeds it the same universe mean_reversion gets. Strict mode via `crypto_runner.require_new_entrant: true` (default false). 12 new tests in `test_crypto_runner.py` + 4 in `test_crypto_scanner.py` for `new_entrants()`. **Pipeline state at handoff:** all 4 PRs merged to origin/main; user hasn't yet (a) pushed their local merge commit (`0afdd38`) — it's only on host's local main, blocking the next `git pull`; (b) flipped `crypto.dynamic_universe.enabled: true` in settings.yaml; (c) run `docker compose up -d --force-recreate trading-bot` to activate the bot/ bind-mount + new code. **Next-session deploy checklist for the user:** `cd /opt/trading-bot && git push origin main && vim config/settings.yaml` (flip the flag) `&& docker compose up -d --force-recreate trading-bot && docker compose logs -f trading-bot | grep -iE "crypto|coingecko|injected|CRYPTO-RUNNER|SLOW CYCLE"`. **Watch tomorrow:** (i) stricter top_gainers thresholds — healthy admit count is 3-8 per scan; if 0-1 across whole premarket, walk thresholds back to 75K/7%; (ii) `SLOW CYCLE 200s+` should be gone (ibkr `_qualified_contracts` cache); (iii) after scanner flip, look for `Injected N crypto symbols` with N≈50 and `CoinGecko: refreshed top-N crypto universe`; (iv) when a runner fires, `CRYPTO-RUNNER SIGNAL:` log line with "new entrant in top-50" in reason if it was a fresh entrant; (v) entry quality — if crypto_runner is making bad entries on tail-end pumps, flip `require_new_entrant: true` for strict mode. **Tests 182/182 (excluding pre-existing broken `test_ghost_position_detection.py`).** **Cleanup deferred:** user's host has stray `.bak` files (`bot/dashboard/templates/dashboard.html.bak.20260522_013652`, `data/positions_state.json.bak.20260518_102746`, `data/trade_history.json.bak.20260518_090525`) + `install.cmd` — flagged but not removed. **Open follow-ups for next session:** (A) scanner scheduling — currently startup-only refresh, daily 00:30 UTC cron would refresh without restarts (one-line `BackgroundScheduler` add); (B) Alpaca-availability check on scanner output — names not listed on Alpaca crypto rails will get signal-log rejections (log noise, not money loss); (C) the auto-deploy cron's `docker compose build --quiet` step is now harmless redundancy with the bot/ bind-mount but could be dropped from `deploy/auto-deploy.sh:234` for ~3-5s faster cycles; (D) consider routing equity scanner names through a similar `new_entrants()` pattern for low_float_catalyst, since the same "first day in top-50" signal is the cleanest "fresh attention" indicator on either asset class.
